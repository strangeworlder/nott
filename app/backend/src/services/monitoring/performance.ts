...
class BackendPerformanceMonitor {
  ...
  trackApiResponse(route: string, responseTime: number, status: number): void {
    ...
    if (this.metrics.apiResponseTimes[route].length > 100) {
      this.metrics.apiResponseTimes[route] = this.metrics.apiResponseTimes[route].slice(-100);
    }

    // Track error rates
    if (status >= 400) {
      this.metrics.apiErrorRates[route] = (this.metrics.apiErrorRates[route] || 0) + 1;
    }
  ...
  trackDbQuery(query: string, queryTime: number): void {
    ...
    if (this.metrics.dbQueryTimes[queryKey].length > 50) {
      this.metrics.dbQueryTimes[queryKey] = this.metrics.dbQueryTimes[queryKey].slice(-50);
    }
  ...
  getPerformanceSummary(): Record<string, unknown> {
    ...
    for (const [route, times] of Object.entries(this.metrics.apiResponseTimes)) {
      if (times.length > 0) {
