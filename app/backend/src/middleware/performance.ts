...
export function performanceMiddleware(
	...
	res.end = function (chunk?: unknown, encoding?: BufferEncoding): Response {
		...
		// Track API response
		backendPerformanceMonitor.trackApiResponse(
			req.path,
			duration,
			res.statusCode,
		);
		...
		if (duration > 1000) {
			...
			logger.info(
				`Moderate request: ${req.method} ${req.path} took ${duration}ms`,
			);
		}
...
export function dbPerformanceMiddleware(
	query: string,
	params: unknown[],
): {
	track: (result: unknown) => unknown;
	trackError: (error: Error) => never;
} {
